 ...
 * f表示加密算法，d表示解密算法 那么从plain得到crypt的过程是: crypt = f(plain &circ; preCrypt) &circ;prePlain 
 * 所以，从crypt得到plain的过程自然是 plain = d(crypt &circ; prePlain) &circ;preCrypt 
...
public class Crypter {
   ...
   // private static Random random = Util.random();
    // 字节输出流
    private  ByteArrayOutputStream baos;   //System.IO.MemoryStream baos;
	
	/**
	 * 构造函数
	 */
	public Crypter() {
        baos = new ByteArrayOutputStream();  //MemoryStream(8);
	}
    ...
    public byte[] encrypt(byte[] In, int offset, int len, byte[] k) {
        ...
        //plain[0] = (byte)((testint & 0xF8) | pos);
        
        // 这里用随机产生的数填充plain[1]到plain[pos]之间的内容
        for(int j = 1; j <= pos; j++)
            plain[j] = (byte)(rand() & 0xFF);
        	//plain[j] = (byte)(testint & 0xFF);
        pos++;
    ...
    private byte[] encipher(byte[] In) {
        ...
        long y = getLong(In, 0, 4);
        long z = getLong(In, 4, 4);
        long a = getLong(key, 0, 4);
        long b = getLong(key, 4, 4);
        long c = getLong(key, 8, 4);
        long d = getLong(key, 12, 4);
        ...
        baos.reset();//.SetLength(0);
        writeInt(y);
        writeInt(z);
        return baos.toByteArray();//.ToArray();
    }

    public long getLong(byte[] In, int offset, int len)
    {
    ...
    private byte[] decipher(byte[] In, int offset) {
        ...
        // 我们用了long，这个long的前32位是全0的，我们通过这种方式模拟无符号整数，后面用到的long也都是一样的
        // 而且为了保证前32位为0，需要和0xFFFFFFFF做一下位与
        long y = getLong(In, offset, 4);
        long z = getLong(In, offset + 4, 4);
        long a = getLong(key, 0, 4);
        long b = getLong(key, 4, 4);
        long c = getLong(key, 8, 4);
        long d = getLong(key, 12, 4);
     ...
     * @param t
     */
    private void writeInt(long t) {
        baos.write((byte)((t >> 24) & 0xFF));
        baos.write((byte)((t >> 16) & 0xFF));
    ...
    /**
     * 通过密钥字符串和加密后的密码字符串得到原始密码字符串
     * 
     * @return
     * 		原始密码字符串
     */
    public static String decryptPassword(String passwordstr,String keystr){
    	
    	///为方便测试
    	///String RetPassword=null;
    	///byte[] k = StringUtils.decodeHex(keystr);
    	///byte[] ret =  StringUtils.decodeHex(passwordstr);
    	///byte[] oo = decrypt(ret, k);
    	
    	///RetPassword = new String(oo);
...
//		 String str = read("e:/test/作品与画像2.pdf");//
//		System.out.println(str);
		 Crypter cr = new Crypter();
		
		// String str = "12345678901234567890";
	        // byte[] data = str.getBytes("UTF-8");
	     byte[] data = readbuf("e:/test/印说.txt");
	         
	     String keystr = "1234567890987654";
	     
	     byte[] key = keystr.getBytes();
	     
	     byte[] ret = cr.encrypt(data, key);
	     
	     write(ret,"e:/test/test_out");
	     
	     ret = readbuf("e:/test/test_out");
	     
	  //   str = Base16.encode(ret); //new String(ret, "UTF-8");
	  //      System.out.println(str);
	     
	     byte[] ooo = cr.decrypt(ret, key);
	     
	 //    str = new String(ooo);
	 //       System.out.println(str);
	     
	     write(ooo,"d:/test/印说.txt");
//	    
//		byte[] data1 = {0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x30,0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x30};
//		byte[] k = {0x31,0x32,0x33,0x34,0x35,0x36,0x37,0x38,0x39,0x30,0x39,0x38,0x37,0x36,0x35,0x34};
//		
//		 byte[] ret1 = cr.encrypt(data1, k);
//		 
//		byte[] ooo1 = cr.decrypt(ret1, k);
//		     
//		 str = Base16.encode(ret1); //new String(ret, "UTF-8");
//	        System.out.println(str);
