...
Ext.extend(MC.Desktop, Ext.util.Observable, {
    ...
    initLaunchers : function(){
    	var l = system.selectReg('launchers');
    	...
    	if(l.contextmenu){
			...
			{
				this.addContextMenuItem(l.contextmenu[i]);
			}
		}
		//初始化桌面图标
		if(l.shortcut){
			for(var i = 0, len = l.shortcut.length; i < len; i++){
	            this.addShortcut(l.shortcut[i], false);
	        }
		}
		//初始化快速启动栏对象
		if(l.quickstart){
	...
	addContextMenuItem : function(id){
		var ms = system.selectReg('modules');
		for(o in ms)
		{
			if(ms[o].moduleId === id)
			{
			...
			this.lockWin = new Ext.Window({
				...
				listeners:{
					...
					items:[{
							...
							html : '<div align="center" style="padding-bottom:15px;">输入挂机密码,请不要与登陆密码相同!</div>'
						},
						{
							xtype:'textfield',
							fieldLabel: '密码',
