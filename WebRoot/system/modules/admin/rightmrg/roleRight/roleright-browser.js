Ext.override(MC.app.Roleright, {
	browser : function(win) {	
		...
		var temp=[
			...
			//{id:'cCreateDate',header: '创建时间', width: 80, dataIndex:'cCreateDate'},
			//{id:'cChangePerson',header: '修改者编号', width:80,  dataIndex:'cChangePerson'},
			//{id:'cChangeDate',header: '修改时间', width: 80, dataIndex:'cChangeDate'}						
		];
		this.roles1= temp;
		var cmConfig = [], sm, tmp;
	...
	qQuery : function(btn) {
		...
		/*var param={roleName:roleName,start : 0,limit : 15};			
 		this.getStore('roles1').setLink('temp');
 		this.getStore('roles1').load({params:param});*/
		var s = this.getStore('roles1');
