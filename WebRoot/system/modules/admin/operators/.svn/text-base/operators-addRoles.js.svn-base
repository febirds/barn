Ext.override(MC.app.Operators, {
	...
	addRoles : function(win) {
		...
		var temp=[
			...
			{id:'roleId',header: '角色编号', width: 160,     dataIndex:'roleId'},
			//{id:'parentRole',header: '父亲角色', width:80,  dataIndex:'parentRole'},
			{id:'roleName',header: '角色名称', width: 160,   dataIndex:'roleName'},
			//{id:'createUser',header: '创建用户', width:80,  dataIndex:'createUser'},
			{id:'createTime',header: '创建时间', width: 200, dataIndex:'createTime'}
		...
		for (var i = 0, len = temp.length; i < len; i++) {
			...
			if ('checkboxSelectionModel'==tmp.xtype) {
				sm = tmp;
			}
		}
		//alert('ff');
		   ...
		   // alert(params);
			Ext.Ajax.request({
				...
				success : function(response,option){					
					...
					//g.getStore().loadData(resObj.grid);
					if(resObj.grid!=null)
					{
						g.getStore().loadData(resObj.grid);// 加载grid
					}					
					this.body.unmask();
				},
				failure : function(){
	...
	//保存修改
	aSave:function(){
		var operId=this.lookupI('operId').getValue();
		...
		var params={operId:operId,rolesIds:rolesIds};
		//adminTwo/role/role.do?method=addOperatorsToRole
		//adminTwo/operator/operator.do?method=queryOperator
		var url="adminTwo/operator/operator.do?method=addRolesToOperator";
