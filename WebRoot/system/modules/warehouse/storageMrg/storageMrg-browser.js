	...
	prepareCMConfig : function(c) {
 	...
 	browser : function(win){
		...
		var viewpanel = new Ext.grid.GridPanel({
			itemId:'storageMrggrid',
			anchor :'100% 0',
			ds : this.getStore('storageMrg'),
			cm : cm,
			//sm : sm,
			border:false,
			height:600,
			...
			bbar : new Ext.PagingToolbar({
						pageSize : 15,
						store : this.getStore('storageMrg'),
						displayInfo : true,
 	...
 	qQuery : function(btn){		
		...
		var param={logName:logName,zhName:zhName,start : 0,limit : 15};			
 		this.getStore('storageMrg').setLink('temp');
 		this.getStore('storageMrg').load({params:param});
 	}
 });
