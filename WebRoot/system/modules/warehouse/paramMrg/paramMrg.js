Ext.ns('MC.app');
MC.app.ParamMrg = Ext.extend(MC.Window, {
 	title:'参数设置',
 	moduleId:'paramMrg',
 	clearer:['paramMrg'],
 	hasStatusBar : true,
 	...
 	createStore : function(id) {
 		if(id === 'paramMrg') {
			var rm = [];
			var tp = this.queryParamMrgStore;
			...
			var queryParamMrgStore = new Ext.data.Store({
				autoLoad : false,
				storeId : 'paramMrg',
				linkId :'base',//这里不写默认的就是'base'
				proxy : new Ext.data.HttpProxy({url : 'admin/paramMrg/paramMrg.do?method=loadAllUser'}),
				reader : new Ext.data.JsonReader({
				totalProperty : 'totalProperty',
				root : 'records'
				}, Ext.data.Record.create(rm))
			});
			queryParamMrgStore.registLinks([{
				linkId : 'temp',
				baseParams : {},
				proxy : new Ext.data.HttpProxy({url : 'admin/paramMrg/paramMrg.do?method=loadAllUser'}),
				reader : new Ext.data.JsonReader({
		...
		} else if(id === 'paramMrgState') {
			new Ext.data.SimpleStore({
				storeId : 'paramMrgState',
				fields : ["text", "value"],
 	...
 	popBrowser : function(btn){
 		...
 		}else if(btn.itemId =='bBtn')
 		{
 			...
 			this.getStore('paramMrg').setLink('base');
 			this.getStore('paramMrg').load({
 				params:{
 					start:0,
	 				limit:15
	 				//,sort:'',
	...
	popModify : function(){
		if (!this.getComponent('ctnr').getComponent('paramMrggrid').getSelectionModel().hasSelection()) {
				this.alert('提示', '请先选择要修改的行!');
				return;
			}
		var userId = this.getComponent('ctnr').getComponent('paramMrggrid').getSelectionModel().getSelected().get('userId');
		this.createWindow({
			width : 400,
			height : 310,
			title : '修改用户',
 	...
 	popDrop : function() {
		...
		this.confirm('确认删除','您真的要删除所选用户吗?',
				function(btn){
					if (btn == 'yes') {
							var record = this.lookupI('paramMrggrid').getSelectionModel().getSelected();// 得到当前选中的记录
							var params = {userId : record.data.userId};
							var url = 'admin/paramMrg/paramMrg.do?method=remove';
							Ext.Ajax.request({
								url : url,
								params : params,
								method : 'POST',
	...
	popStart : function() {
		...
		this.confirm("提示", "确实要启用该用户吗？", function(btn) {
			if (btn == 'yes') {				
				var record = this.getComponent('ctnr').getComponent('paramMrggrid').getSelectionModel().getSelected();// 得到当前选中的记录
				var params = {userId : record.data.userId,
					paramMrgState : record.data.paramMrgState
				};
				var url = 'admin/paramMrg/paramMrg.do?method=activate';
				Ext.Ajax.request({
					url : url,
					params : params,
					method : 'POST',
	...
	popStop : function() {
		...
		this.confirm("提示", "确实要停用该用户吗？", function(btn) {
			if (btn == 'yes') {
				...
				var url = 'admin/paramMrg/paramMrg.do?method=cancel';
				Ext.Ajax.request({
					url : url,
					params : params,
					method : 'POST',
	...
	//格式化用户状态
	paramMrgStateFormat : function(value) {
		switch (value) {
			case '0' :
				return "停用";
				break;
 ...
 MC.app.ParamMrg.singleTon = true;
 Ext.onReady(function(){
  window.system.install('paramMrg',MC.app.ParamMrg);
 });
