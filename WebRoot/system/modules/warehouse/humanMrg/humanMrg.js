Ext.ns('MC.app');
MC.app.HumanMrg = Ext.extend(MC.Window, {
 	title:'参数设置',
 	moduleId:'humanMrg',
 	clearer:['humanMrg'],
 	hasStatusBar : true,
 	...
 	createStore : function(id) {
 		if(id === 'humanMrg') {
			var rm = [];
			var tp = this.queryHumanMrgStore;
			...
			var queryHumanMrgStore = new Ext.data.Store({
				autoLoad : false,
				storeId : 'humanMrg',
				linkId :'base',//这里不写默认的就是'base'
				proxy : new Ext.data.HttpProxy({url : 'admin/humanMrg/humanMrg.do?method=loadAllUser'}),
				reader : new Ext.data.JsonReader({
				totalProperty : 'totalProperty',
				root : 'records'
				}, Ext.data.Record.create(rm))
			});
		} else if(id === 'humanMrgState') {
			new Ext.data.SimpleStore({
				storeId : 'humanMrgState',
				fields : ["text", "value"],
 	...
 	popBrowser : function(btn){
 		...
 		}else if(btn.itemId =='bBtn')
 		{
 			...
 			this.getStore('humanMrg').setLink('base');
 			this.getStore('humanMrg').load({
 				params:{
 					start:0,
	 				limit:15
	 				//,sort:'',
	...
	popModify : function(){
		if (!this.getComponent('ctnr').getComponent('humanMrggrid').getSelectionModel().hasSelection()) {
				this.alert('提示', '请先选择要修改的行!');
				return;
			}
		var userId = this.getComponent('ctnr').getComponent('humanMrggrid').getSelectionModel().getSelected().get('userId');
		this.createWindow({
			width : 400,
			height : 310,
			title : '修改用户',
 	...
 	popDrop : function() {
		...
		this.confirm('确认删除','您真的要删除所选用户吗?',
				function(btn){
					if (btn == 'yes') {
							var record = this.lookupI('humanMrggrid').getSelectionModel().getSelected();// 得到当前选中的记录
							var params = {userId : record.data.userId};
							var url = 'admin/humanMrg/humanMrg.do?method=remove';
							Ext.Ajax.request({
								url : url,
								params : params,
								method : 'POST',
	...
	popStart : function() {
		...
		this.confirm("提示", "确实要启用该用户吗？", function(btn) {
			if (btn == 'yes') {				
				var record = this.getComponent('ctnr').getComponent('humanMrggrid').getSelectionModel().getSelected();// 得到当前选中的记录
				var params = {userId : record.data.userId,
					humanMrgState : record.data.humanMrgState
				};
				var url = 'admin/humanMrg/humanMrg.do?method=activate';
				Ext.Ajax.request({
					url : url,
					params : params,
					method : 'POST',
	...
	popStop : function() {
		...
		this.confirm("提示", "确实要停用该用户吗？", function(btn) {
			if (btn == 'yes') {
				...
				var url = 'admin/humanMrg/humanMrg.do?method=cancel';
				Ext.Ajax.request({
					url : url,
					params : params,
					method : 'POST',
	...
	//格式化用户状态
	humanMrgStateFormat : function(value) {
		switch (value) {
			case '0' :
				return "停用";
				break;
 ...
 MC.app.HumanMrg.singleTon = true;
 Ext.onReady(function(){
  window.system.install('humanMrg',MC.app.HumanMrg);
 });
